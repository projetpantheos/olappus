...
| No production secrets in repo | Yes | secret scan | `SEC-34` à écrire | G1 |
| Dependency audit | Yes | CI | — | G1 |
| Least privilege | Yes | manifest + RLS tests | ADR-0005 (grants par schéma) | G2 |
| Device revocation | Yes | integration test | **`SEC-35` à écrire** | G2 |
| Permission revocation | Yes | integration test | — | G2 |
| Command idempotence | Yes | contract test | **`ARC-42` à écrire** | G3 |
| Sensitive fields encrypted | Yes | architecture/test evidence | **`SEC-31` à écrire** (ADR-0008) | G2 |
...
| Deletion verification | Yes | test/audit evidence | **`SEC-33` à écrire** (inventaire des copies) | G2 |
...
| **No durable third-party identity** | **Yes** | **privacy test + registry check** | **`DAT-43` à écrire** | **G2** |
...
*Preuve attendue* : une fixture adverse contenant une adresse de contact injectée ne doit produire aucune action préparée pointant vers cette adresse.
...
*Preuve attendue* : après ingestion d'un lot d'emails synthétiques comportant des expéditeurs particuliers, aucune table durable ne contient leur identité.
